...
import org.renthouse.common.TokenCache;
...
import org.renthouse.util.MD5Util;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public ServerResponse<String> register(User user,Integer type) {
        ...
        if(type == 1){
            //身份是房东
            user.setRole(UserConst.Role.ROLE_OWNER);
        }else if((type == 2)){
            //身份是租客
            user.setRole(UserConst.Role.ROLE_RENTER);
        }
        //MD5加密
    ...
    public ServerResponse selectQuestion(String username) {
        ServerResponse validResponse = this.checkValid(username,UserConst.USERNAME);
        if(validResponse.isSuccess()){
            //用户不存在
    ...
    public ServerResponse checkAdminRole(User user) {
        if(user != null && user.getRole().intValue() == UserConst.Role.ROLE_ADMIN){
            return ServerResponse.createBySuccessMessage("此用户是管理员");
        }
